===============
Authentication
===============

Contents:

.. toctree::
   :maxdepth: 1

Base URL
----------------
Transfocus TMS API is REST based API with multiple secure end points. All resources reside under base URL. One of the base URL given below can be used to access TMS API:: 

    https://api.tmspanel.com.au/
    https://tms-api-dev.azurewebsites.net/


Authentication Endpoint
------------------------
TMS API is secured uing OAuth tokens. Tokens are JSON Web Token (JWT). JWT token can be obtained by sending POST request to token URL::

    POST https://tms-api-dev.azurewebsites.net/oauth2/token

Example Request
----------------
Transfocus TMS API - example authentication request is given below::

    curl -POST -H "Content-type: application/json" -d 'username:$username
    password:$password
    grant_type:password' 'https://tms-api-dev.azurewebsites.net/oauth2/token'
  
Example Response
----------------
Transfocus TMS API - example authentication response is given below::

    {
        "access_token": "eyJhbGciOiJodHRwOi8vd3d3LnczLm9yZy8yMDAxLzA0L3htbGRzaWctbW9yZSNobWFjLXNoYTI1NiIsInR5cCI6IkpXVCJ9..edtL-TztKjNPpvaUQ13wRIOIpAg0ojWKSs8KjWWCtrc",
        "token_type": "bearer",
        "expires_in": 43199
    }

This access_token will be used in further API calls to access secure endpoints.

Example Errors
----------------
TMS API can send back following standard HTTP errors response:

.. list-table:: **Error Codes and Description**
   :widths: 25 75
   :header-rows: 1

   * - Error Code
     - Description
   * - 200 OK
     - Request is successful.
   * - 400 Bad Request
     - Request parameters are incorrect.
   * - 401 Unauthorized
     - Authentication failed.
   * - 402 Request Failed
     - Request is failing.
   * - 403 Forbidden
     - You don’t have permissions to access that URL.
   * - 404 Not Found 
     - The requested resource doesn't exist on server.
   * - 500 Server Error
     - There is server error in API.

References
----------------

* JSON Web Tokens can be verified online using JWT tool::

    https://jwt.io/#debugger-io

